...
DELETE FROM sales
WHERE invoice_number = '7628231837' -- An employee was recently fired so we must delete them from our database. Delete the employee with employee_id of 35. What problems might you run into when deleting? How would you recommend fixing it?
        ---------------------------------
UPDATE employees
SET isACtive = false
...
ADD COLUMN IF NOT EXISTS sACtive bool Default true
ALTER TABLE employees
...
SELECT *
FROM vehicles
WHERE vehicle_id = 2 --Creating a stored procedure to remove a vehicle from inventory whenever a vehicle is sold
        -------------------------------------------------------------
...
END $$;
CALL remove_vehicle_from_inventory(1) ------------------------------------------------------------
--Creating a stored procedure to return a sold vehicle
------------------------------------------------------------
CREATE PROCEDURE return_sold_vehicle(in vehicleId int) LANGUAGE plpgsql AS $$ BEGIN
UPDATE sales
SET returned = true
...
AFTER
INSERT ON sales FOR EACH ROW EXECUTE PROCEDURE set_purchase_date();
--------------------------------------
-- QUESTION #2
---------------------------------------------------------
...
AFTER
INSERT ON sales FOR EACH ROW EXECUTE PROCEDURE set_pickup_date();
--------------------------------------------------------
-- QUESTION #3
...
UPDATE ON sales FOR EACH ROW EXECUTE PROCEDURE conditionally_set_pickup_date();
--------------------------------------------------
...
------------------------------------------------------------------------------------------
CREATE FUNCTION format_dealership_website() RETURNS TRIGGER LANGUAGE PlPGSQL AS $$ BEGIN -- trigger function logic
NEW.website := CONCAT(
...
INSERT
        OR
UPDATE ON dealerships FOR EACH ROW EXECUTE PROCEDURE format_dealership_webiste();
------------------------------------------------------------------------------------------
-- QUESTION #2
...
------------------------------------------------------------------------------------------
CREATE FUNCTION default_phone_number()
RETURNS TRIGGER 
LANGUAGE PLPGSQL
AS $$
BEGIN
...
WHERE d.phone IS NULL AND d.dealership_id = NEW.dealership_id;
RETURN NULL;

END;
$$

CREATE trigger add_default_phone_number
	AFTER INSERT
	ON dealerships
	for each row
  EXECUTE PROCEDURE default_phone_number();
